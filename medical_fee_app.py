...
def predict_page():
    ...
    if submitted:
        ...
        # 初始化数据预处理格式中与岛屿相关的变量（书本笔误，忽略“岛屿”描述）
        ...
        #'rfr_model.pkl'待放到streamlit_env文件中
        with open('rfr_model.pkl', 'rb') as f:  # 修复：原代码错写为CSV文件，改为模型文件
            rfr_model = pickle.load(f)

        # 将格式后的数据转为DataFrame（匹配模型的输入特征格式）
        # 修复：补全缺失的右括号，移除多余的if submitted（避免缩进错误）
